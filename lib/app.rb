...
#puts client.update ("YOOOO ! Premier tweet avec ruby")
#puts client.update("Un petit deuxieme pour la route parceque c'est kiffant #thp")
...
def like_follow(client_stream,client)
 ...
 like_follow(@client_stream, client)
...
def newArray(array, client)

	new_array = []

	new_array << array.sample(5)
	journa = new_array.flatten

	for handle in journa 

		puts client.update(" Bonjour #{handle}, allez jeter un oeil a @the_hacking_pro #bisous")

	end

end
 #newArray(array,client)

 def like_tweet(client)

 	client.search("#bonjour_monde", result_type: "recent").take(25).each do |tweet|
  puts client.favorite(tweet)
	end
end
...
def follow_bonjour(client)
	client.search("#bonjour_monde", result_type: "recent").take(14).each do |tweet|
